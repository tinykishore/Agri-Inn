<script lang="ts">
    ...
    import seenNotification from "$lib/assets/notificationIcon/readNotification.svg";
    import saveIcon from "$lib/assets/icons/save.svg";
    import unseenNotification from "$lib/assets/notificationIcon/unreadNotification.svg";
...
<div class="grid grid-cols-3 items-center">
	<div class="flex justify-start gap-2">
		...
		<a href="/search"
		   class="px-6 py-2 flex text-sm items-center gap-2 align-middle rounded-full font-bold border border-yellow-800 hover:bg-amber-300 text-yellow-950 transition-all duration-300"><span
				class="block h-5 w-5"><img src={search_icon} alt="search_icon"></span>
			Search Anywhere
		</a>
	</div>
	...
	<div class="flex justify-end gap-2 text-sm">
		<div class="flex justify-start gap-3">
			<a href="/saved-item"
			   class="px-3 py-2 text-sm flex items-center align-middle rounded-full font-bold border border-yellow-800 hover:bg-amber-300 text-yellow-950 transition-all duration-300">
				<span
						class="block h-5 w-5 mr-2">
				<img class="" alt="search_icon" src={saveIcon}></span>
				Saved Items
			</a>
		</div>
		...
		{#if isUnseen}
			<button on:click={showNotification}
			   class=" flex gap-4 rounded-full align-middle justify-center items-center font-semibold text-yellow-950  hover:bg-yellow-300 hover:border-yellow-800 transition-all duration-300">
				<img class="h-9 w-9" src={unseenNotification} alt="">
			</button>
		{:else}
			<button on:click={showNotification}
			   class="flex gap-4 rounded-full align-middle justify-center items-center font-semibold text-yellow-950  hover:bg-yellow-300 hover:border-yellow-800 transition-all duration-300">
